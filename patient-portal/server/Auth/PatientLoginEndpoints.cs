...
namespace PatientPortalServer.Auth;

// Subdomain is optional: omit it to let the server figure out (or ask about)
// which tenant, or pass one back from a prior MultipleTenantsResponse to
// complete the login. There's no separate "does this username exist" lookup
// — that was a free, unauthenticated username-existence oracle. Figuring out
// which tenant(s) a username belongs to now requires submitting a password.
public record LoginRequest(string Username, string Password, string? Subdomain);

// ClientOrigin is the tenant's own browser-reachable client — patient-portal
// never hosts a session itself, it proxies the credential check then hands
// the browser off to whichever tenant actually owns it. See the redirect
// this response is used to build in patient-portal/client's LoginPage.
...
public static class PatientLoginEndpoints
{
    public const string LoginProxyClientName = "tenant-login-proxy";

    public static void MapPatientLoginEndpoints(this WebApplication app)
    {
        ...
        .WithName("PatientLogin")
        .AllowAnonymous();
    }

    // Proxies the credential check to the tenant's own /api/auth/login and
    // relays its outcome. Patient-portal never stores or verifies passwords
    // itself — the tenant's server is the only thing that can.
