...
export function LoginPage() {
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // This site never hosts the session itself — hand the browser off to
      // the tenant's own client, which owns everything from here on (welcome
      // page, logout, whatever else it wants). Cross-origin, so a real
      // navigation, not client-side routing. Token goes in the URL fragment
      // (not a query string) so it's never sent to a server or logged.
